...
def display_bounding_boxes(text_map, output_size=(1280, 800), image=None):
    ...
    if image is not None:
        background = cv2.resize(
            image, (output_size[0], output_size[1])
        ...
        background = np.zeros(
            (output_size[1], output_size[0], 4), dtype=np.uint8
    ...
    colors = {
        text: (0, 255, 0, 255) for _, text in text_map
    }  # Green colour for all boxes
    ...
    for corners, text in text_map:
        ...
        color = colors[text]
        cv2.polylines(
            background, [corners.astype(int)], isClosed=True, color=color, thickness=2
        )
...
def display_corner_aruco_markers(
    image,
    ...
    for i, pos in enumerate(aruco_positions):
        ...
        marker_resized = cv2.resize(
            marker_image, (marker_size, marker_size)
        )  # Ensure correct size
        marker_bgr = cv2.cvtColor(marker_resized, cv2.COLOR_GRAY2BGR)
        marker_bgra = cv2.merge(
            (marker_bgr, np.full((marker_size, marker_size), 255, dtype=np.uint8))
        )  # Add alpha channel

        # Place the ArUco marker on the background (using BGRA format)
        image[pos[1] : pos[1] + marker_size, pos[0] : pos[0] + marker_size] = (
            marker_bgra
        )
...
def display_bounding_box(image, corners, color=(0, 255, 0), thickness=2):
    ...
    """
    cv2.polylines(
        image, [corners.astype(int)], isClosed=True, color=color, thickness=thickness
    )
    return image


def render_code_in_bbox(
    image, code, bbox, font_scale=0.6, font=cv2.FONT_HERSHEY_SIMPLEX, thickness=1
):
    """
...
def display_projection(
    ...
    error_box = (
        scale_bounding_boxes([(np.array(error_box), "")], input_size, output_size)[0][0]
    ...
    image = display_corner_aruco_markers(
        image,
