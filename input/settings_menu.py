...
class SettingsMenu:
    ...
    def update_camera_dropdown(self):
        ...
        self.camera_dropdown["values"] = [
            f"{i}: {name}" for i, name, _, _ in self.cameras
        ]
    ...
    def create_device_settings_tab(self):
        ...
        self.camera_dropdown = ttk.Combobox(
            main_frame, values=["Detecting cameras..."], width=50
        )
        self.camera_dropdown.grid(row=row, column=1, columnspan=4, sticky="ew", pady=2)
        self.camera_dropdown.bind(
            "<<ComboboxSelected>>", lambda e: self.update_resolution_fields()
        )
        row += 1
        ...
        self.cam_res_width = ttk.Entry(
            self.cam_res_frame, width=7, validate="key", validatecommand=vcmd
        )
        ...
        ttk.Checkbutton(
            main_frame, text="Enable Voice Commands", variable=self.voice_commands_var
        ...
        ttk.Label(main_frame, text="Projection Resolution:").grid(
            row=row, column=0, sticky="w", pady=2
        )
        ...
        ttk.Checkbutton(
            main_frame, text="Project Image", variable=self.project_image_var
        ).grid(row=row, column=0, sticky="w", pady=2)
        row += 1
        ...
        ttk.Checkbutton(
            main_frame, text="Project Corner Markers", variable=self.project_corners_var
        ).grid(row=row, column=0, sticky="w", pady=2)
        row += 1

        # Corner marker size
        ttk.Label(main_frame, text="Corner Marker Size:").grid(
            row=row, column=0, sticky="w", pady=2
        )
        ...
        ttk.Label(main_frame, text="Detection Confidence:").grid(
            row=row, column=0, sticky="w", pady=2
        )
        ...
        # Code Save Path
        ttk.Label(main_frame, text="Code Save Path:").grid(
            row=row, column=0, sticky="w", pady=2
        )
    ...
    def create_helper_code_tab(self):
        ...
        save_btn = ttk.Button(
            self.helper_frame, text="Save All Settings", command=self.on_save
        )
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", "Please enter valid numbers in all numeric fields."
                )
                return
